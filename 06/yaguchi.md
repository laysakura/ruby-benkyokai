...
>
> [メタプログラミング - Wikipedia](http://ja.wikipedia.org/wiki/%E3%83%A1%E3%82%BF%E3%83%97%E3%83%AD%E3%82%B0%E3%83%A9%E3%83%9F%E3%83%B3%E3%82%B0)
...
>
> [Rubyの生産性の高さはどこまで本当か？ - 分裂勘違い君劇場](http://d.hatena.ne.jp/fromdusktildawn/20061002/1159784863)

## マクロ (C言語)
* コンパイル前処理（プリプロセス）時に、機械的な文字列の置き換えを行う
* 引数を取ることができる
...
## メソッドの動的な呼び出し
* Object#send
    * 文字列やシンボルで指定したメソッドを呼び出す
    * 第1引数にメソッド名、第2引数以降に本来のメソッドの引数を渡す
...
* Module#alias_method
    * あるメソッドのエイリアスを定義する
    * 第1引数に新しいメソッド名を、第2引数にすでにあるメソッドの名前を渡す
        * 代入文と同じ順序と覚えると良い
...
* 呼び出したメソッドが存在しなかった時、最終的に呼び出されるメソッド
	* 第1引数にメソッド名、第2引数以降にそのメソッドの引数が渡される
